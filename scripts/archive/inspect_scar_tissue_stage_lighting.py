...
def light_paths(node):
    if node is None:
        return []
    stage_node = node if hasattr(node, "stage") else node.input(0)
    if stage_node is None or not hasattr(stage_node, "stage"):
        return []
...
portrait_lights = set(render_info["portrait_9x16_settings"]["light_prims"])
if portrait_lights != expected_lights:
    raise RuntimeError(
        "portrait render branch does not contain the final lighting rig: "
        f"expected {sorted(expected_lights)}, got {sorted(portrait_lights)}"
    )
